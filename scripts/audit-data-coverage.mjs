...
const broadReferenceSourceIds = new Set(["ec-calendar", "eufic-europe", "fao-crop-calendar", "fao-infoods", "langual", "usda-fdc"]);
const hasCountrySpecificReference = (entry) =>
  entry.sourceIds.some((sourceId) => !broadReferenceSourceIds.has(sourceId));
...
const priorityCoverage = priorityCountries.map((country) => {
  ...
  return {
    ...
    countrySpecificRefs: overrides.filter(hasCountrySpecificReference).length,
...
- Country/item overrides with country-specific references: ${countryOverridePairs.filter(hasCountrySpecificReference).length}
...
${priorityCoverage
  .map(
    (entry) =>
      ...
      `, ${entry.countrySpecificRefs} with country-specific references` +
  ...
  JSON.stringify(
    {
      ...
      countrySpecificReferencePairs: countryOverridePairs.filter(hasCountrySpecificReference).length,
